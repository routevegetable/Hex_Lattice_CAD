...
class TileRef:
    ...
    @staticmethod
    def from_xy(graph: Graph, x: int, y: int) -> "TileRef":
        return TileRef(graph, x, y)
    ...
    def negate(self) -> "TileRef":
        return TileRef(self.graph, -self.x, -self.y)

    def add(self, tr: "TileRef") -> "TileRef":
        return TileRef(self.graph, (self.x + tr.x) % self.graph.width, (self.y + tr.y) % self.graph.height)
...
class VertexRef:
    ...
    def ends_cw(self) -> Tuple["EndRef", "EndRef", "EndRef"]:
        ...
        s = self.tile.graph.VERTEX_END_SETS[self.vertex_class]
...
class EdgeRef:
    ...
    def vertex_pair(self) -> "EdgeVertexPair":
        p = self.tile.graph.EDGE_VERTEX_PAIRS[self.edge_class]
...
class HexGrid:
    ...
    def __getitem__(self, key: tuple[int, int]) -> HexRef:
        x,y = key
        if y % 2 == 0:
            vertex = TileRef(self.graph, x, y // 2).vertex(VertexClass.DEF)
        else:
            vertex = TileRef(self.graph, x + 1, y // 2).vertex(VertexClass.ABC)
...
class TileGrid:
    ...
    def __getitem__(self, key: tuple[int, int]) -> TileRef:
        return TileRef(self.graph, key[0], key[1])
...
class VertexGrid:
    ...
    def __getitem__(self, key: tuple[int, int]) -> VertexRef:
        ...
        match (y % 4):
            case 0:
                return TileRef(self.graph, x, y // 4).vertex(VertexClass.DEF)
            case 1:
                return TileRef(self.graph, x, y // 4).vertex(VertexClass.CDE)
            case 2:
                return TileRef(self.graph, x, y // 4).vertex(VertexClass.ABC)
            case _:
                return TileRef(self.graph, x, y // 4).vertex(VertexClass.ABF)
...
class Graph:
    ...
    def __init__(self, width: int, height: int):
        self.width = width
        self.height = height
        self.VERTEX = VertexGrid(self)
        self.TILE = TileGrid(self)
        self.HEX = HexGrid(self)
        
        # Tile Offsets
        self.THIS_TILE = TileRef(self, 0, 0)
        self.LEFT_TILE = TileRef(self, -1, 0)
        self.RIGHT_TILE = TileRef(self, 1, 0)
        self.UP_TILE = TileRef(self, 0, 1)
        self.DOWN_TILE = TileRef(self, 0, -1)


        self.VERTEX_END_SETS: Dict[VertexClass, EndSet] = {
            VertexClass.ABC: EndSet(
                v=self.THIS_TILE.top_end(EdgeClass.C),
                l=self.LEFT_TILE.bottom_end(EdgeClass.B),
                r=self.THIS_TILE.bottom_end(EdgeClass.A),
            ),
            VertexClass.ABF: EndSet(
                v=self.UP_TILE.bottom_end(EdgeClass.F),
                l=self.THIS_TILE.top_end(EdgeClass.A),
                r=self.THIS_TILE.top_end(EdgeClass.B),
            ),
            VertexClass.CDE: EndSet(
                v=self.THIS_TILE.bottom_end(EdgeClass.C),
                l=self.LEFT_TILE.top_end(EdgeClass.E),
                r=self.THIS_TILE.top_end(EdgeClass.D),
            ),
            VertexClass.DEF: EndSet(
                v=self.THIS_TILE.top_end(EdgeClass.F),
                l=self.THIS_TILE.bottom_end(EdgeClass.D),
                r=self.THIS_TILE.bottom_end(EdgeClass.E),
            ),
        }

        self.EDGE_VERTEX_PAIRS: Dict[EdgeClass, EdgeVertexPair] = {
            EdgeClass.A: EdgeVertexPair(VertexRef(VertexClass.ABF, self.THIS_TILE),
                                        VertexRef(VertexClass.ABC, self.THIS_TILE)),
            EdgeClass.B: EdgeVertexPair(VertexRef(VertexClass.ABF, self.THIS_TILE),
                                        VertexRef(VertexClass.ABC, self.RIGHT_TILE)),
            EdgeClass.C: EdgeVertexPair(VertexRef(VertexClass.ABC, self.THIS_TILE),
                                        VertexRef(VertexClass.CDE, self.THIS_TILE)),
            EdgeClass.D: EdgeVertexPair(VertexRef(VertexClass.CDE, self.THIS_TILE),
                                        VertexRef(VertexClass.DEF, self.THIS_TILE)),
            EdgeClass.E: EdgeVertexPair(VertexRef(VertexClass.CDE, self.RIGHT_TILE),
                                        VertexRef(VertexClass.DEF, self.THIS_TILE)),
            EdgeClass.F: EdgeVertexPair(VertexRef(VertexClass.DEF, self.THIS_TILE),
                                        VertexRef(VertexClass.ABF, self.DOWN_TILE)),
        }

    def tiles(self) -> Generator[TileRef, None, None]:
        for x in range(self.width):
            for y in range(self.height):
                yield self.TILE[x, y]

    def edges(self) -> Generator[EdgeRef, None, None]:
        for tile in self.tiles():
            for c in EdgeClass:
                yield tile.edge(c)
    
    def ends(self) -> Generator[EndRef, None, None]:
