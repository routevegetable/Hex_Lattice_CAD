...
def trace(
    now: Event, trigger: Event, base: EndRef, path: str, period: int, rate: float
) -> Generator[tuple[float, EndRef]]:
...
while True:
    ...
    for end in graph.ends():
        ...
        for i in range(4):
            ...
            saturation = vary(now, 0.955, 1, PERIOD * 3, i / 4)
    ...
    if brightness == 0:
        ...
        for d, end in trace(
            now,
            beat_event.read(),
            graph.TILE[0, 0].bottom_end(EdgeClass.E),
            "LRLRLLL",
            300,
            200,
        ):
